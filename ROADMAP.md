...
- Phase 23A: Local Scan History Feature (lib/features/shared/domain/models/scan_history_entry_model.dart — ScanHistoryEntry model; lib/features/history/presentation/screens/scan_history_screen.dart — HistoryScreen with empty state, history list, clear history, history entry cards; lib/features/scan/presentation/screens/scan_screen.dart — history entry creation after successful analysis; lib/features/passport/presentation/screens/profile_screen.dart — "Scan History" preference tile with navigation; lib/features/shared/data/mock_repositories.dart — scanHistoryProvider; session-local history only, no persistence, no backend/config/secrets changed)
- Phase 23B: Scan History Feature Tests (test/helpers/test_models.dart — test model factories; 4 new test files: scan_history_entry_model_test.dart (14 tests), scan_history_provider_test.dart (9 tests), scan_screen_history_test.dart (4 tests), scan_history_screen_test.dart (4 tests); total 73/73 tests pass; fixed HistoryScreen + ProfileScreen imports; no backend/config/secrets changed)
- Phase 24A: Saved Dietary Preferences and Allergen Settings (DietaryPreferencesModel with allergen toggles + dietary restrictions; shared_preferences persistence across app restarts; DietaryPreferencesScreen for editing preferences; ProfileScreen "Dietary Preferences" entry point; ResultCard allergen match warning badge + "Contains:" info; disclaimer banner; 23 new tests; total 96/96 tests pass; dietary_preferences_provider.dart with load/save/clear; no iOS/backend/.env/secrets changes)
- Phase 24B: Preferences UX Polish and Product Smoke Test (ProfileScreen: icons on tiles, "Quick Actions" section, dynamic subtitles; ResultCard: consolidated allergen badge into single "Contains [allergen]" badge; ScanHistoryScreen: deprecation + const fixes; lint clean; 1 new multi-allergen test; total 97/97 tests pass; PHASE_24B_REPORT.md; no iOS/backend/.env/secrets changes)
- Phase 25A: Demo Product Flow Script and Manual QA Checklist (DEMO_PRODUCT_FLOW_SCRIPT.md: 6-act demo flow with 60s/2min/detailed narration scripts; MANUAL_QA_CHECKLIST.md: 74-point QA checklist across environment/preferences/scan/history/safety/regression; DEMO_RECORDING_SHOT_LIST.md: 10-shot recording guide (81s); README.md and ROADMAP.md updated; documentation-only, no code changes; no iOS/backend/pubspec/secrets changes)
- Phase 25B: Portfolio Demo Package (PORTFOLIO_DEMO_PACKAGE.md: complete portfolio overview with pitch, architecture, talking points, and honest limitations; GITHUB_REPO_SHOWCASE_CHECKLIST.md: 10-section repo showcase checklist; README.md: added Current MVP Features, Demo Flow, What is Real vs Mock, Safety and Limitations, How to Run, Portfolio Value sections; ROADMAP.md updated; documentation-only, no code changes; no iOS/backend/pubspec/secrets changes)
- Phase 25C: Public GitHub Repository Final QA (PUBLIC_REPO_FINAL_QA.md: 9-section QA document covering README review, link QA, safety wording QA, repository presentation QA, technical honesty QA, demo readiness QA, known limitations, final verdict; 1 minor gap identified — scan history session-local clarification; all internal links verified; repository confirmed portfolio-ready; ROADMAP.md updated; documentation-only, zero code changes; dart analyze 54 pre-existing info-level only; flutter test 97/97 passing; no lib/ios/backend/pubspec/secrets/provider changes)
- Phase 26A: Public Flutter Web Demo Deployment (flutter build web --release with --base-href /ai-food-passport/demo/ and --dart-define=BACKEND_BASE_URL=https://ai-food-passport.onrender.com; build successful in 39.6s; 37 files, ~41 MB copied to docs/demo/; PUBLIC_WEB_DEMO_DEPLOYMENT.md and PHASE_26A_REPORT.md created; README.md: added Public Web Demo section with link, caveats, and known limitations; ROADMAP.md updated; no changes to lib/ios/backend/pubspec/secrets/providers; dart analyze 54 pre-existing info-level only; flutter test 97/97 passing; public demo URL pending GitHub Pages propagation after push)
- Phase 26B: Public Web Demo Live Verification (all 4 public URLs confirmed live via HTTP: demo 200 OK, privacy policy 200 OK, Render health 200 OK / mock-only, API endpoint reachable; base-href verified correct; PUBLIC_WEB_DEMO_LIVE_VERIFICATION.md and PHASE_26B_REPORT.md created; ROADMAP.md updated; verification-only, zero code changes; dart analyze 54 pre-existing info-level only; flutter test 97/97 passing; manual browser verification 22/22 passed; no lib/ios/backend/pubspec/secrets/provider changes)
- Phase 27A: Release v0.2 Portfolio Demo Ready (RELEASE_V0_2_0_PORTFOLIO_DEMO_READY.md — complete release notes with status, links, features, real vs mock, verification, limitations, safety disclaimers, next phase; GITHUB_RELEASE_V0_2_0_DRAFT.md — copy-paste-ready GitHub Release body with tag v0.2.0-portfolio-demo-ready, key features, verification summary, limitations, safety disclaimer; PHASE_27A_REPORT.md — phase report with all verification data; README.md and ROADMAP.md updated; documentation-only, zero code changes; dart analyze 54 pre-existing info-level only; flutter test 97/97 passing; productionReady unchanged (false); no real providers enabled; no lib/ios/backend/pubspec/secrets/provider changes)
- Phase 27C: Release v0.2.0 Verification (verified published GitHub Release: page reachable 200 OK, tag v0.2.0-portfolio-demo-ready confirmed, title "AI Food Passport v0.2.0 — Portfolio Demo Ready" correct; safety wording review PASS — all sensitive terms only as explicit disclaimers; public web demo 200 OK; privacy policy 200 OK; Render backend health 200 OK mock-only, realProvidersEnabled:false, productionReady:false; RELEASE_V0_2_0_VERIFICATION.md — 10-section verification with 11-criterion final verdict all PASS; PHASE_27C_REPORT.md — full phase report; README.md updated with GitHub Release URL + verification report link; ROADMAP.md updated; verification-only — zero code changes; dart analyze 54 info-level only; flutter test 97/97; no lib/ios/backend/pubspec/secrets/provider changes; final verdict: VERIFIED — release safe to share publicly)
- Phase 28A: Public Sharing Package (PUBLIC_SHARING_PACKAGE.md — master 9-section sharing doc: one-line pitch, 30-sec/2-min explanations, key links, features, tech highlights, honest limitations, use-case cross-reference; RESUME_PROJECT_BLURB.md — 5 resume blurb versions with honest wording notes; LINKEDIN_POST_DRAFT.md — professional LinkedIn post in full + short versions with posting tips; DCARD_POST_DRAFT.md — Chinese conversational Dcard post in full + short + tech-board versions; GITHUB_PROFILE_PINNED_REPO_TEXT.md — repo About/topics/captions/badges with how-to-apply; PHASE_28A_REPORT.md — full phase report; ROADMAP.md updated; documentation-only — zero code changes; dart analyze 54 info-level only; flutter test 97/97; no lib/ios/backend/pubspec/secrets/provider changes)
- Phase 28B: GitHub Showcase Setup Verification (GITHUB_SHOWCASE_SETUP.md — 8-section showcase doc: repo identity, About description confirmed accurate, Website URL confirmed, 10 Topics all honest, pinned repo status documented with pinned caption limitation accepted as GitHub UI constraint, safety & honesty review all PASS, configuration summary, suggested future actions; PHASE_28B_REPORT.md — full phase report with 12-criterion final verdict all PASS; ROADMAP.md updated; documentation-only — zero code changes; dart analyze 54 info-level only; flutter test 97/97; no lib/ios/backend/pubspec/secrets/provider changes; productionReady remains false)
- Phase 29A: License + Repo Legal Hygiene (LICENSE — standard MIT License, Copyright 2026 AllengWong-droid, no custom clauses; README.md — added License section linking to LICENSE; PHASE_29A_REPORT.md — full phase report; ROADMAP.md updated; documentation-only — zero code changes; dart analyze 54 info-level only; flutter test 97/97; no lib/ios/backend/pubspec/secrets/provider changes; productionReady remains false)
