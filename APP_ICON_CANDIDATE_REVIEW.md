...
| **3. Silhouette test** | Scale to 29x29, 40x40, 60x60, 120x120, 180x180. Verify legibility at each size. | ✅ Done (Phase 22C) — Generated preview images at 1024/512/180/120/60/40px + contact sheet. Reviewed readability: excellent (1024–180px), good (120px), acceptable (60px), borderline (40px). See `APP_ICON_QA_SMALL_SIZE_VALIDATION.md`. |
| **4. Acceptance sign-off** | Run through the full 16-item acceptance checklist again with the actual exported image. | ✅ Done (Phase 22C) — Full QA: 15 visual/safety checks pass, acceptance scorecard 19/20. Accepted as **design-source asset** only. NOT final iOS production icon (needs macOS + square re-export). See `APP_ICON_QA_SMALL_SIZE_VALIDATION.md` Section 7. |
| **5. Commit PNG master to repo** | Add the final 1024x1024 PNG to `docs/design-assets/app_icon_source_v1.png`. This is the first binary image asset in the repo. | ✅ Done (Phase 22E) — Clean square master ingested at `design/app-icon/source/ai-food-passport-clean-square-master.png` (1254x1254, RGB opaque, 2.56 MB). Phase 22C issue RESOLVED. Accepted as preferred export source (21/21 validation pass). See `APP_ICON_CLEAN_SQUARE_MASTER_INTAKE.md`. |
